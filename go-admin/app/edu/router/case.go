...
func registerCaseRouter(v1 *gin.RouterGroup, authMiddleware *jwt.GinJWTMiddleware) {
	...
	r := v1.Group("/edu/cases").Use(authMiddleware.MiddlewareFunc()).Use(middleware.AuthCheckRole())
	{
		...
		r.PUT("/:id", api.Update)
		r.DELETE("", api.Delete)
		r.PUT("/:id/submit-review", api.SubmitReview)
		r.PUT("/:id/review", api.Review)
		r.GET("/:id/reviews", api.GetReviews)
		r.GET("/:id/attachments", api.GetAttachments)
		r.POST("/:id/attachments", api.AddAttachment)
		r.DELETE("/:id/attachments", api.DeleteAttachments)
		r.GET("/:id/attachments/:attachmentId/file-url", api.GetAttachmentURL)
		r.GET("/:id/access-logs", api.GetAccessLogs)
		r.GET("/:id/access-logs/export", api.ExportAccessLogs)
