...
func (e EduRecruit) PublicCompanies(c *gin.Context) {
	...
	order := "id desc"
	if req.Sort == "jobs" {
		order = "(select count(1) from edu_recruit_job where edu_recruit_job.company_id = edu_recruit_company.id and edu_recruit_job.deleted_at is null) desc,id desc"
	}
...
func (e EduRecruit) CreateCompany(c *gin.Context) {
	req := models.EduRecruitCompany{}
	if err := e.MakeContext(c).MakeOrm().Bind(&req, binding.JSON).Errors; err != nil {
		e.Error(500, err, err.Error())
		return
	}
	if strings.TrimSpace(req.CompanyName) == "" || strings.TrimSpace(req.CreditCode) == "" {
		e.Error(400, nil, "企业名称和统一社会信用代码不能为空")
		return
	}
	...
	if req.ReviewStatus == "" || req.ReviewStatus == "pending" {
		req.ReviewStatus = "approved"
	}
	if req.CertStatus == "" || req.CertStatus == "pending" {
		req.CertStatus = "approved"
	}
	if req.ReviewedAt == "" && req.ReviewStatus == "approved" {
		req.ReviewedAt = now
	}
	req.SetCreateBy(user.GetUserId(c))
	if err := e.Orm.Create(&req).Error; err != nil {
...
func (e EduRecruit) CreateJob(c *gin.Context) {
	req := models.EduRecruitJob{}
	if err := e.MakeContext(c).MakeOrm().Bind(&req, binding.JSON).Errors; err != nil {
		e.Error(500, err, err.Error())
		return
	}
	if strings.TrimSpace(req.JobName) == "" || req.CompanyId == 0 {
		e.Error(400, nil, "岗位名称和所属企业不能为空")
		return
	}
	var company models.EduRecruitCompany
	if err := e.Orm.First(&company, req.CompanyId).Error; err != nil {
		e.Error(400, err, "所属企业不存在")
		return
	}
	req.CompanyName = company.CompanyName
	if req.Industry == "" {
	...
	if req.PublishTime == "" && req.Status == "published" {
		req.PublishTime = now
	}
	if req.ReviewedAt == "" && req.ReviewStatus == "approved" {
		req.ReviewedAt = now
...
func (e EduRecruit) Stats(c *gin.Context) {
	...
	result["incompleteCompanies"] = countRecruit(e.Orm, &models.EduRecruitCompany{}, "status = ?", "incomplete")
	...
	monthTotal := countRecruit(e.Orm, &models.EduRecruitCompany{}, "created_at >= ?", monthStart) + countRecruit(e.Orm, &models.EduRecruitJob{}, "created_at >= ?", monthStart)
	monthPassed := countRecruit(e.Orm, &models.EduRecruitCompany{}, "review_status = ? and reviewed_at >= ?", "approved", monthStart) + countRecruit(e.Orm, &models.EduRecruitJob{}, "review_status = ? and reviewed_at >= ?", "approved", monthStart)
	result["monthApplications"] = monthTotal
	result["monthPassed"] = monthPassed
	e.OK(result, "查询成功")
}
...
func applyRecruitCompanyFilters(db *gorm.DB, req recruitCompanyQuery) *gorm.DB {
	...
	if req.EndDate != "" {
		db = db.Where("created_at < ?", nextDate(req.EndDate))
	}
	return db
}
...
func applyRecruitJobFilters(db *gorm.DB, req recruitJobQuery) *gorm.DB {
	...
	if req.StartDate != "" {
		db = db.Where("(publish_time >= ? or (publish_time = '' and created_at >= ?))", req.StartDate, req.StartDate)
	}
	if req.EndDate != "" {
		end := nextDate(req.EndDate)
		db = db.Where("(publish_time < ? or (publish_time = '' and created_at < ?))", end, end)
	}
	return db
}
